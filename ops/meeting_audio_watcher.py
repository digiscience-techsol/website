...
def rec_key(path):
    parts = path.parts
    hits = [i for i, part in enumerate(parts) if part == "recordings"]
    try:
        i = hits[-1]
...
def stable_files(files):
    ...
    for path in files:
        try:
            ...
            continue
        if stat.st_size > 2500 and time.time() - stat.st_mtime > 4:
            out.append(path)
    return out
...
def concat_audio(files, output):
    ...
    try:
        subprocess.run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", list_file, "-vn", "-c:a", "libopus", str(output)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=120, check=True)
...
def process_group(model, key, files):
    ...
    if not concat_audio(files, combined):
        return
    text = transcribe_file(model, combined)
    group["signature"] = signature
    if text and len(text) >= 5 and not SILENCE_RE.match(text):
        delta = new_delta(group.get("text", ""), text)
        group["text"] = text
        if delta:
            append_transcript(key, delta)
            send_telegram(f"Live transcript [{key}]: {delta}")
            if "?" in delta or QUESTION_RE.search(delta):
...
def main():
    ...
    send_telegram("Direct audio watcher is online: local Whisper will transcribe combined Vexa audio chunks and send Telegram alerts.")
    while True:
        try:
            groups = {}
            for path in all_audio_files():
                groups.setdefault(rec_key(path), []).append(path)
            for key, files in groups.items():
                try:
                    process_group(model, key, files)
                except Exception as exc:
                    print("group_error", key, repr(exc), flush=True)
            save_state()
        except Exception as exc:
