# Founder Commercial Approval Form

Status: Internal founder approval form. No decision is approved yet.
Created: 2026-05-28

## Purpose

Record the minimum founder approvals required before DigiScience can quote, propose, invoice, and start an AI Readiness Assessment.

## 1. Pricing Decision

### Option A: Low-Friction First Customer

- Price range: INR 75,000-1,25,000 / USD 900-1,500.
- Scope: smallest paid assessment; 1 workflow, 1-2 stakeholder calls, lightweight readiness scorecard, pilot go/no-go recommendation.
- Risk: underpricing effort; limited room for deeper security/governance work.
- Recommendation: use if the goal is fastest first paid customer and buyer has procurement friction.
- Founder decision: Pending.
- Why needed: a proposal cannot be quoted without approved price.
- Who must decide: founder.
- What happens if not decided: DigiScience cannot safely send a quote or invoice.
- 45-day revenue impact: high.

### Option B: Standard Assessment

- Price range: INR 2,00,000-3,50,000 / USD 2,400-4,200.
- Scope: normal assessment; 1 priority workflow, 2-4 stakeholder calls, workflow/data/risk/governance review, readiness scorecard, pilot scope, final readout.
- Risk: higher approval friction and slower first close.
- Recommendation: use for serious buyers with stakeholder access and budget signal.
- Founder decision: Pending.
- Why needed: creates standard commercial anchor for qualified buyers.
- Who must decide: founder.
- What happens if not decided: pricing remains ambiguous and proposal turnaround slows.
- 45-day revenue impact: high.

### Option C: Assessment Credited Toward 45-Day Pilot

- Price range: INR 1,50,000-2,50,000 / USD 1,800-3,000.
- Scope: readiness assessment with pilot conversion logic, success criteria, security/governance guardrails, and pilot recommendation.
- Risk: credit can reduce near-term margin and requires clear expiry/credit terms.
- Recommendation: recommended default for a serious buyer that may convert to pilot.
- Founder decision: Pending.
- Why needed: lets DigiScience position assessment as a risk-reduced path to pilot.
- Who must decide: founder.
- What happens if not decided: credit logic cannot be included in proposal.
- 45-day revenue impact: high.

## 2. Proposal Owner

### Option A: DigiBot Drafts, Founder Approves

- Founder decision: Pending.
- Why needed: establishes 24-hour proposal workflow.
- Who must decide: founder.
- What happens if not decided: proposal ownership remains unclear after a qualified response.
- 45-day revenue impact: high.

### Option B: Founder Drafts, DigiBot Formats

- Founder decision: Pending.
- Why needed: gives founder direct commercial wording control.
- Who must decide: founder.
- What happens if not decided: proposal drafting may wait on ad hoc coordination.
- 45-day revenue impact: medium.

### Option C: Shared Workflow

- Founder decision: Pending.
- Why needed: splits founder commercial input and DigiBot drafting/formatting.
- Who must decide: founder.
- What happens if not decided: response-to-proposal SLA remains soft.
- 45-day revenue impact: medium-high.

## 3. Billing Owner

### Option A: Founder Handles Billing

- Founder decision: Pending.
- Why needed: fastest manual billing route if no tool is configured.
- Who must decide: founder.
- What happens if not decided: invoice ownership remains unclear.
- 45-day revenue impact: high.

### Option B: DigiBot Prepares Invoice Draft

- Founder decision: Pending.
- Why needed: speeds invoice preparation after founder provides billing details.
- Who must decide: founder.
- What happens if not decided: DigiBot cannot prepare invoice-ready draft fields.
- 45-day revenue impact: medium-high.

### Option C: Accounting Tool Used

- Founder decision: Pending.
- Why needed: enables formal invoice generation if tool exists.
- Who must decide: founder.
- What happens if not decided: accounting-tool path cannot be used.
- 45-day revenue impact: medium.

## 4. Payment Method

Recommended current path: manual invoice + company bank transfer.

### Option A: Bank Transfer

- Founder decision: Pending.
- Why needed: gives buyer a clear payment path.
- Who must decide: founder.
- What happens if not decided: accepted proposal cannot convert to payment request.
- 45-day revenue impact: high.
- Current status: Recommended current path when paired with manual invoice.

### Option B: UPI

- Founder decision: Pending.
- Why needed: may reduce payment friction if appropriate for business payment.
- Who must decide: founder.
- What happens if not decided: UPI cannot be offered.
- 45-day revenue impact: medium.

### Option C: Razorpay

- Founder decision: Deferred.
- Current status: DEFERRED; not an active default.
- Why needed: payment link can simplify collection later if setup exists.
- Who must decide: founder.
- What happens if not decided: Razorpay remains deferred and cannot be offered.
- 45-day revenue impact: medium-high.

### Option D: Stripe

- Founder decision: Deferred.
- Current status: DEFERRED; not an active default.
- Why needed: payment link option later if setup exists and is appropriate.
- Who must decide: founder.
- What happens if not decided: Stripe remains deferred and cannot be offered.
- 45-day revenue impact: medium.

### Option E: Other

- Founder decision: Pending.
- Why needed: allows founder-specified payment route.
- Who must decide: founder.
- What happens if not decided: no alternate payment route exists.
- 45-day revenue impact: medium.

## 5. Signature Method

### Option A: Email Acceptance

- Founder decision: Pending.
- Why needed: lowest-friction acceptance route.
- Who must decide: founder.
- What happens if not decided: acceptance evidence remains unclear.
- 45-day revenue impact: high.

### Option B: Signed PDF

- Founder decision: Pending.
- Why needed: formal signature route if buyer requires it.
- Who must decide: founder.
- What happens if not decided: formal acceptance may stall.
- 45-day revenue impact: medium-high.

### Option C: E-Sign

- Founder decision: Pending.
- Why needed: structured signature workflow if tool is available.
- Who must decide: founder.
- What happens if not decided: e-sign route cannot be offered.
- 45-day revenue impact: medium.

### Option D: PO

- Founder decision: Pending.
- Why needed: supports procurement-led buyers.
- Who must decide: founder.
- What happens if not decided: PO-dependent buyer may not proceed to kickoff.
- 45-day revenue impact: medium-high.

## Approval Summary

Minimum approvals needed before sending a proposal:

- Pricing option and exact price/range.
- Proposal owner.
- Billing owner.
- Payment method. Recommended current path: manual invoice + company bank transfer.
- Signature method.

Current commercial lock references:

- [Founder Commercial Decisions Locked](founder-commercial-decisions-locked.md)
- [Manual Invoice Bank Transfer Readiness](manual-invoice-bank-transfer-readiness.md)

Sensitive bank/tax details must not be stored in Git. If founder-led retrieval is needed, use a temporary noVNC session through a secure private path, founder logs in manually, DigiBot records only non-secret workflow status, and the public tunnel is closed after use.

## No-Hallucination Confirmation

No option is approved in this form. No quote, proposal, invoice, payment, signature, customer acceptance, or billing start is claimed.
